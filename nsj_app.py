...
LECTURER_ADVANCE_PAYMENT_DATE = date(2020, 12, 31)  # 前金支付日，只影响现金流说明

# 其他日期 / その他の日程
WEB_AD_START = date(2021, 3, 12)
PAMPHLET_PRINT_DATE = date(2021, 2, 26)
PAMPHLET_SEND_DATE = TIMING_B
RECRUIT_START = date(2021, 4, 26)
RECRUIT_END = date(2021, 5, 14)
PARTICIPANT_FIXED_DATE = TIMING_C
VENUE_FULL_PAYMENT_DATE = TIMING_C
...
STAFF_MAN_DAYS = 20
DEFAULT_STAFF_MONTHLY_SALARY = 420_000
...
def calculate_staff_cost(include_staff_cost, staff_monthly_salary):
    if not include_staff_cost:
        staff_cost = 0
        staff_process = L(
            "本次设定为不计算员工工资，因此员工工资 = 0 円",
            "今回は社内従業員給与を計算しない設定のため、社内従業員給与 = 0 円"
        )
    else:
        staff_daily_cost = staff_monthly_salary / WORKING_DAYS_PER_MONTH
        staff_cost = staff_daily_cost * STAFF_MAN_DAYS
        staff_process = L(
            f"计算员工工资：{yen(staff_monthly_salary)} ÷ {WORKING_DAYS_PER_MONTH}日 × {STAFF_MAN_DAYS}人日 = {yen(staff_cost)}",
            f"社内従業員給与を計算：{yen(staff_monthly_salary)} ÷ {WORKING_DAYS_PER_MONTH}日 × {STAFF_MAN_DAYS}人日 = {yen(staff_cost)}"
        )

    return staff_cost, staff_process


# =========================================================
# 7. 成本组件计算 / Cost Components
# =========================================================

def calculate_component_rows(
    final_people,
    ...
    # 3. 员工工资 / 社内従業員給与
    normal_staff, normal_staff_process = calculate_staff_cost(include_staff_cost, staff_monthly_salary)

    if cancel_date is not None:
        if not include_staff_cost:
            cancel_staff = 0
            cancel_staff_process = L(
                "本次设定为不计算员工工资，因此取消时员工工资 = 0 円",
                "今回は社内従業員給与を計算しない設定のため、キャンセル時の社内従業員給与 = 0 円"
            )
        else:
            ...
            cancel_staff = normal_staff * ratio
            cancel_staff_process = L(
                f"取消时按准备进度估算员工工资：{yen(normal_staff)} × {ratio:.2f} = {yen(cancel_staff)}",
                f"キャンセル時点までの準備進捗で社内従業員給与を見積もる：{yen(normal_staff)} × {ratio:.2f} = {yen(cancel_staff)}"
            )
    else:
        cancel_staff = None
...
with st.sidebar:
    ...
    if include_staff_cost:
        staff_monthly_salary = st.slider(
            L("员工月工资（日元）", "従業員月給（円）"),
            min_value=300_000,
            max_value=600_000,
            value=DEFAULT_STAFF_MONTHLY_SALARY,
            step=1_000
        )
    else:
        staff_monthly_salary = 0
    ...
    if include_staff_cost:
        st.write(L(f"员工月工资：{yen(staff_monthly_salary)}", f"従業員月給：{yen(staff_monthly_salary)}"))
    else:
        st.write(L("员工工资：不计算", "社内従業員給与：計算しない"))
